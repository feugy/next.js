...
export async function ncc_edge_runtime_feature_detector(task, opts) {
  // `@edge-runtime/feature-detector` is precompiled and pre-bundled
  // so we vendor the package as it is.
  const dest = 'src/compiled/@edge-runtime/feature-detector'
  const pkg = await fs.readJson(
    require.resolve('@edge-runtime/feature-detector/package.json')
  )
  await fs.remove(dest)
  ...
  await fs.copy(
    require.resolve('@edge-runtime/feature-detector'),
    join(dest, 'index.js')
  )
  await fs.copy(
